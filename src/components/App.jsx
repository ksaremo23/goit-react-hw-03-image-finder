...
class App extends Component {
  ...
  async componentDidUpdate(_prevProps, prevState) {
    const { searchQuery, prevState } = this.state;
  ...
  handleSearchSubmit = query => {
    ...
    if (normalizedQuery === '') {
      alert(`Empty string is not a valid searchquery. Please try again.`);
      return;
    }

    if (normalizedQuery === normalizedCurrentQuery) {
      alert(
        `Search query is the same as the previous one. Please type again.`
      );
      return;
    }
  ...
  render() {
    ...
    return (
      <div>
        ...
        {isError && <p>Somethingwent wrong. Please try again later.</p>}
      </div>
    );
  }
}
